...
async function loadCharacters() {
    try {
        const characters = await apiCall('/api/sanguo/characters');
...
async function startNewGame() {
    try {
        const result = await apiCall('/api/sanguo/new-game', 'POST');
        if (result && !result.error) {
            gameState.currentAnswer = result.answer_id;
...
async function makeGuess() {
    ...
    try {
        const result = await apiCall('/api/sanguo/guess', 'POST', {
